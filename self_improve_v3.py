"""
Self-improvement v3 — duration-weighted scoring.

Key changes vs pin_sa_optimise:
  - Score ignores notes shorter than MIN_SCORE_DUR (30ms) in both
    chroma and DTW components. Optimizer must match the melodic
    backbone, not micro-ornament fragments.
  - SUB_WINDOWS 4 → 6  (more scoring windows per cycle)
  - DTW_SAMPLES  24 → 36 (finer time resolution)
  - Sa pinned at 96.97 Hz, same as before.
"""
...
MIN_SCORE_DUR = 0.030   # notes shorter than this don't count in the score
MIN_DUR_FLOOR = 0.005   # optimizer still free to go fine; scoring filters them
...
def synth_chroma(notes, t0, t1):
    """Only count notes whose overlap with this window is >= MIN_SCORE_DUR."""
    h = np.zeros(12)
    for start,dur,sw,_ in notes:
        ov = min(start+dur,t1) - max(start,t0)
        if ov < MIN_SCORE_DUR: continue
...
def synth_seq(notes, t0, t1, n):
    """Only count notes >= MIN_SCORE_DUR in duration when building the sequence."""
    step = (t1-t0)/n
    seq = []
    for i in range(n):
        tc = t0+(i+0.5)*step
...
# ── Baseline ──────────────────────────────────────────────────
print("="*62)
print(f"  SELF-IMPROVE v3 — duration-weighted scoring")
print(f"  Sa={SA_HZ} Hz  sub_windows={SUB_WINDOWS}  dtw_samples={DTW_SAMPLES}")
print(f"  MIN_SCORE_DUR={MIN_SCORE_DUR*1000:.0f}ms  (micro-notes excluded from score)")
print("="*62)

prev = json.load(open("best_params.json"))
print(f"\nWarm-start: score={prev['best_score']:.5f}  "
      f"conf={prev['confidence_threshold']}  "
      f"gap={prev['gap_tolerance']}  min_dur={prev['min_duration']}")
...
long_notes = sum(1 for _,d,_,_ in baseline_notes if d >= MIN_SCORE_DUR)
print(f"Baseline (v3 metric) : {baseline_score:.5f}  "
      f"({len(baseline_notes)} notes, {long_notes} ≥30ms)  [{time.time()-t0:.1f}s]")
...
for rnd, (dc,dg,dm,pop,mi) in enumerate(rounds, 1):
    ...
    if rnd_sc > best_score + 0.0002:
        ...
        long_n = sum(1 for _,d,_,_ in best_notes if d >= MIN_SCORE_DUR)
        print(f"  → New best!  conf={conf0}  gap={gap0}  min_dur={md0}"
              f"  notes={len(best_notes)}  long≥30ms={long_n}")
    else:
...
print(f"  Score (v3)           : {best_score:.5f}")
print(f"  Notes (total)        : {len(best_notes)}")
print(f"  Notes (≥30ms)        : {long_n}  ({100*long_n/len(best_notes):.1f}%)")
print(f"  Evaluations          : {eval_n[0]}")

# ── Write notation ─────────────────────────────────────────────
def write_notation(notes, outfile="sargam_notation_cleaned.txt"):
    lines = [
        "SARGAM NOTATION — TANPURA-ANCHORED v3 (self_improve_v3.py)",
        "="*65,
        f"Sa = {SA_HZ} Hz  (tanpura harmonic analysis)",
        f"Confidence = {conf}  Gap = {gap}s  Min duration = {md}s",
        f"Score = {best_score:.5f}  (v3: duration-weighted, 6 sub-windows, 36 DTW samples)",
        f"Total notes: {len(notes)}  (≥30ms: {long_n})", "",
        "KEY:",
...
result = {
    ...
    "score_version"         : "v3_duration_weighted",
